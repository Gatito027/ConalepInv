...
import { EliminarItem } from "../../infrastructure/EliminarItem";

export default function EliminarItemModal({ itemid, setShowModal, reload }) {
  const [isLoading, setIsLoading] = useState(false);
  const handleDelete = async (e) =>{
    e.preventDefault();
    setIsLoading(true);
    try {
      const response = await EliminarItem({ itemId: itemid });
      ...
      console.error(error);
      toast.error("Error: No se ha podido eliminar el item");
    } finally {
      setIsLoading(false);
      setShowModal(false);
    }
