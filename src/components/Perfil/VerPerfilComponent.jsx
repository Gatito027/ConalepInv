...
export default function VerPerfilComponent() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const response = await RecuperarPerfil();
    ...
    <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-lg overflow-hidden mt-20">
      ...
      <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex flex-wrap gap-3 justify-end">
        <button
          ...
          className="px-4 py-2 text-gray-600 hover:text-gray-800 hover:bg-gray-200 rounded-lg transition-colors flex items-center gap-2"
        >
        ...
        <button
          ...
          className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg transition-colors flex items-center gap-2 shadow-md hover:shadow-lg"
        >
      ...
      {showPasswordModal && (
        <RestablecerPasswordModal usuarioId={user.usuarioId} setShowModal={setShowPasswordModal} />
      )}
    </div>
  );
}
