...
import { EliminarRol } from "../../infrastructure/EliminarRol";

export default function EliminarRolModal({ idrol, reload, setShowModal }) {
  const [isLoading, setIsLoading] = useState(false);
  const handleDelete = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    try {
      const response = await EliminarRol({ rolId: idrol });
      ...
      console.error(error);
      toast.error("Error: No se ha podido eliminar el rol");
    } finally {
      setIsLoading(false);
      setShowModal(false);
    }
