...
import { ObtenerRolPermisos } from "../../infrastructure/ObtenerRolPermisos";
...
export default function EditarRolComponent({ rolId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await EditarRol(rolId, nombreRol, selectedPermisos);
      if (response.isSuccess) {
        toast.success("Información del rol actualizado");
      } else {
  ...
  const fetchPermisos = useCallback(async () => {
    try {
      ...
      const perm = await ObtenerRolPermisos(rolId);

      if (!perm.isSuccess || !perm.data) {
        setNoData(true);
      }
      //console.log(perm.data[0]);
      setNombreRol(perm.data[0].nombre);
      ...
      if (perm.isSuccess && response.data?.length) {
        setSelectedPermisos(perm.data[0].permisos);
      }
    } catch (error) {
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Redefine los permisos y características del rol
              </p>
            </div>
          </div>
