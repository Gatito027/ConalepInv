...
import UbicacionModal from "../NuevoItem/UbicacionModal";
import ModeloModal from "../NuevoItem/ModeloModal";
import MarcaModal from "../NuevoItem/MarcaModal";
import CuentaModal from "../NuevoItem/CuentaModal";
...
export default function EditarItemComponent({ itemId }) {
  ...
  const [bajaUrl, setBajaUrl] = useState("");
  const [polizaUrl, setPolizaUrl] = useState("");
  const [error, setError] = useState(false);
  const [borrarPoliza, setBorrarPoliza] = useState(false);
  const [borrarBaja, setBorrarBaja] = useState(false);

  const formattedDateConvertor = (fecha) => {
    if (fecha === null) return;
    // 1. Crear un objeto Date
    const date = new Date(fecha);

    // 2. Convertirlo a formato YYYY-MM-DD
  ...
  const handleSubmit = async (e) => {
    ...
    appendIfValid(formData, "_borrarBaja", borrarBaja);
    appendIfValid(formData, "_borrarPoliza", borrarPoliza);
    appendIfValid(formData, "_ItemId", itemId);

    try {
      const response = await ActualizarArchivo(formData, "inv/editar-bien");
  ...
  const ObtenerDatos = useCallback(async () => {
    try {
      ...
      setSubcuenta(response.data.subcuentaamonizada);
      setEstado(response.data.estado);
      setNumeroinv(response.data.numeroinventario);
      setCodigoPartida(response.data.codigodepartida);
      setUbicacion(response.data.lugarid);
      setObservaciones(response.data.observaciones);
      setcantidad(response.data.cantidad);
      setMarca(response.data.marcaid);
      setModelo(response.data.modeloid);
      setSerie(response.data.numeroserie);
      setValorLibros(response.data.valorlibros);
      setDespreciacion(response.data.despreciacion);
      setCuenta(response.data.cuenta);
      setDepartamento(response.data.cuentaid);
      setValor(response.data.cotizacion);
      setFechaAlta(formattedDateConvertor(response.data.fechaalta));
      setVidaUtil(response.data.vidautil);
      setDonativo(response.data.donativo);
      setCostoAdquision(response.data.costoadquision);
      setFechaAdquision(formattedDateConvertor(response.data.fechaadquision));
      setFechaResguardo(formattedDateConvertor(response.data.fecharesguardo));
      setMotivoResguardo(response.data.motivoresguardo);
      setTipoBaja(response.data.tipobaja);
      setFechaBaja(formattedDateConvertor(response.data.fechabaja));
      setPolizaFecha(formattedDateConvertor(response.data.fechapoliza));
      setPolizaDate(formattedDateConvertor(response.data.fechadocumentopoliza));
      setImgUrl(response.data.imagenurl);
      setBajaUrl(response.data.documentobaja);
      setPolizaUrl(response.data.documentopoliza);
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 tracking-tight">
                Editar el item: {descripcion}
              </h1>
              <p className="text-sm text-gray-500 mt-1">
                Redefine las características del item: {descripcion}
              </p>
            </div>
          </div>
        </div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        setArchivo({
                          ...
                          file: documentoBaja ? documentoBaja : bajaUrl,
                        });
                        setShowPdf(true);
                      }}
                  ...
                  <label className="flex items-center cursor-pointer">
                    <div className="relative">
                      <input
                        type="checkbox"
                        checked={borrarBaja}
                        onChange={(e) => setBorrarBaja(e.target.checked)}
                        className="sr-only peer"
                      />
                      <div className="w-10 h-5 bg-gray-300 rounded-full peer peer-checked:bg-emerald-500 transition-colors"></div>
                      <div className="absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow peer-checked:translate-x-5 transition-transform"></div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        setArchivo({
                          ...
                          file: polizaDocumento ? polizaDocumento : polizaUrl,
                        });
                        setShowPdf(true);
                      }}
                      className="text-emerald-700 hover:underline text-sm"
                ...
                <div className="relative">
                  <label className="flex items-center cursor-pointer">
                    <div className="relative">
                      <input
                        type="checkbox"
                        checked={borrarPoliza}
                        onChange={(e) => setBorrarPoliza(e.target.checked)}
                        className="sr-only peer"
                      />
                      <div className="w-10 h-5 bg-gray-300 rounded-full peer peer-checked:bg-emerald-500 transition-colors"></div>
                      <div className="absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow peer-checked:translate-x-5 transition-transform"></div>
